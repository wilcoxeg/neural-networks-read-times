...
logLik_test <- function(lm, data) {
  predictions <- predict(lm, data, re.form=NA)
  # Get fitted estimated std.dev.
  stdev <- sigma(lm)
  # For each prediction--observation, get the density p(obs | N(predicted, model_sigma)) and reduce
  density <- dmvnorm(data, predictions, as.matrix(stdev), log=True)
  return(density)
}
```
