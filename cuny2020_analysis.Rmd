...
baselines = train_data %>%
  group_by(training, model, seed, corpus) %>%
    summarise(baseline_lik = logLik(lm(psychometric ~ len + freq + sent_pos, data=.), REML=F),
              baseline_test_lik = logLik_test(lm(psychometric ~ len + freq + sent_pos, data=.), semi_join(test_data, ., by=c("training", "model", "seed", "corpus")), semi_join(test_data, ., by=c("training", "model", "seed", "corpus"))$psychometric),
              #baseline_test_mse = mse_test(lm(psychometric ~ len + freq + sent_pos, data=.), semi_join(test_data, ., by=c("training", "model", "seed", "corpus")), semi_join(test_data, ., by=c("training", "model", "seed", "corpus"))$psychometric)
              ) %>%
  ungroup()
baselines

# Nicely grouped version
# d_lik_pre = train_data %>%
#   group_by(model, training, seed, corpus) %>%
#   {
#     {this_lm <- lm(psychometric ~ surprisal + len + freq + sent_pos, data=.);
#      this_test_data <- semi_join(test_data, ., by=c("training", "model", "seed", "corpus"))} %>%
#     summarise(log_lik = logLik(this_lm, REML = F),
...
d_lik_pre = train_data %>%
  group_by(model, training, seed, corpus) %>%
    summarise(log_lik = logLik(lm(psychometric ~ surprisal + len + freq + sent_pos, data=.), REML = F),
              test_lik = logLik_test(lm(psychometric ~ surprisal + len + freq + sent_pos, data=.), semi_join(test_data, ., by=c("training", "model", "seed", "corpus")), semi_join(test_data, ., by=c("training", "model", "seed", "corpus"))$psychometric),
              #test_mse = mse_test(lm(psychometric ~ surprisal + len + freq + sent_pos, data=.), semi_join(test_data, ., by=c("training", "model", "seed", "corpus")), semi_join(test_data, ., by=c("training", "model", "seed", "corpus"))$psychometric)
              ) %>%
  ungroup() %>%
  mutate(seed = as.factor(seed))
...
full_baselines = all_data %>%
  group_by(model, training, seed, corpus) %>%
  summarise(baseline_train_all_test_lik = logLik_test(lm(psychometric ~ len + freq + sent_pos, data=.), semi_join(test_data, ., by=c("training", "model", "seed", "corpus")), semi_join(test_data, ., by=c("training", "model", "seed", "corpus")))) %>%
  ungroup()
full_baselines
...
full_baselines %>%
  ...
  select(-baseline_test_mse, -baseline_lik) # %>%
...
# Compute residuals from metric ~ PPL relationship, and try to explain those
d_corp$resid.ppl <- resid(lm(delta_test_metric ~ training:test_ppl, data=d_corp))
