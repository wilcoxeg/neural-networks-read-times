...
fit_gam_inner = function(bootstrap_sample, key) {
  ...
  newdata = newdata %>%
    mutate(y=predict(m, newdata=., type="link",
                     exclude=c("te(freq, len, bs = 'cr')",
                               "te(prev_freq, prev_len, bs = 'cr')",
                               "te(prev2_freq, prev2_len, bs = 'cr')",
                               "te(prev3_freq, prev3_len, bs = 'cr')")))
...
fit_gam = function(df, key, alpha=0.05) {
  ...
  boot_models = df %>% bootstraps(times=100) %>% 
...
smooths = read.csv("../data/gam_smooths.csv")
