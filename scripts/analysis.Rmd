...
model_deltas %>%
  ...
  ggplot(aes(x=test_ppl, y=delta_test_mean, color=training_vocab, fill = training_vocab, ymin=0)) +
    ...
    theme(axis.text=element_text(size=12),
          ...
          legend.position = "right")
ggsave("../images/cuny2020/ppl_loglik.png",height=5,width=12)
...
# Fit a GAM for the given model--training--corpus group.
fit_gams = function(df, key) {
  ...
  m.pred = predict(m, newdata=newdata, se.fit=TRUE, type="link",
                   exclude=c("te(freq, len, bs = 'cr')","te(prev_freq, prev_len, bs = 'cr')","te(prev2_freq, prev2_len, bs = 'cr')", "te(prev3_freq, prev3_len, bs = 'cr')"))
...
smooths = all_data %>%
  mutate(
    training_vocab=as.factor(ifelse(str_detect(training, "gptbpe"), "gptbpe", as.character(training))),
    training_source=as.factor(str_replace(as.character(training), "-gptbpe", ""))) %>%
  group_by(training_source, model, corpus) %>%
    group_modify(fit_gams) %>% 
  ungroup()
...
smooths %>%
  ...
  ggplot(aes(x=surprisal, y=y)) +
      theme_bw() +
      geom_line(size=0.5, aes(color=training, linetype=bpe)) +
      ...
      ylab("Reading Time") +
      ggtitle("Surprisal vs. Reading Time") +
...
smooths %>%
  ...
  ggplot(aes(x=surprisal, y=y, fill=training_source, linetype=model)) +
      theme_bw() +
      geom_line(size=0.5, aes(color=training_source)) +
      #geom_line(aes(y=y_lower), linetype="dashed") +
...
ggsave("../images/cuny2020/gam_surp_corr_full.png", height=5,width=12)
