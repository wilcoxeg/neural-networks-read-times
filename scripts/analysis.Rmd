...
ideal_token_obs_count = all_data %>% group_by(corpus, code) %>% group_size() %>% max()
...
loginfo(paste("Dropping", nrow(to_drop), "observations corresponding to corpus tokens which have surprisal zeros for some model."))
loginfo(paste("Dropping", to_drop %>% group_by(corpus, code) %>% n_groups(), "tokens which have surprisal zeros for some model."))
...
```{r}
# TODO: There are a huge number of missing rows for natural stories. What's going on?
# There are only ~11k tokens in the corpus in the first place ..

```
 
```{r}
missing_rows %>% filter(corpus == "natural-stories", training == "bllip-lg-gptbpe") # %>% group_by(code, word) %>% summarise(n=n())
missing_rows %>% filter(corpus == "natural-stories", model == "gpt2") %>% group_by(code, word) %>% summarise(n=n())
```

```{r}
all_data %>% filter(corpus == "natural-stories", code > 10800, code < 10810) %>% group_by(code, word) %>% summarise(n=n()) %>% arrange(code)
```
