...
baseline_corpus = function(corpus, df, test_data, env) {
  if(corpus == "dundee") {
    get_lm_data(df, test_data, psychometric ~ te(freq, len, bs = "cr") + te(prev_freq, prev_len, bs = "cr"), env)
  } else {
    get_lm_data(df, test_data, psychometric ~ te(freq, len, bs = "cr") + te(prev_freq, prev_len, bs = "cr") + te(prev2_freq, prev2_len, bs = "cr"), env)
  }
}

full_model_corpus = function(corpus, df, test_data, env) {
  if(corpus[1] == "dundee") {
    get_lm_data(df, test_data, psychometric ~ s(surprisal, bs = "cr", k = 20) + s(prev_surp, bs = "cr", k = 20) + te(freq, len, bs = "cr") + te(prev_freq, prev_len, bs = "cr"), env)
  } else {
    get_lm_data(df, test_data, psychometric ~ s(surprisal, bs = "cr", k = 20) + s(prev_surp, bs = "cr", k = 20) + s(prev2_surp, bs = "cr", k = 20) + te(freq, len, bs = "cr") + te(prev_freq, prev_len, bs = "cr") + te(prev2_freq, prev2_len, bs = "cr"), env)
  }
}
