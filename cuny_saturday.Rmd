...
folds <- cut(seq(1,nrow(all_data)),breaks=10,labels=FALSE)
...
baselines = baseline_results %>%
  group_by(model, training, seed, corpus) %>%
    summarise(log_lik = mean(log_lik),
              test_lik = mean(test_lik),
              test_mse = mean(test_mse)) %>%
  ungroup() %>%
  mutate(seed=as.factor(seed))

full_models = full_model_results %>%
  group_by(model, training, seed, corpus) %>%
    summarise(log_lik = mean(log_lik),
              test_lik = mean(test_lik),
              test_mse = mean(test_mse)) %>%
...
d_lik = full_models %>%
  ...
  # Compute delta(some metric). (Choose likelihood vs. MSE here.)
  ...
  #select(-test_lik.full, -test_lik.baseline) %>%
...
```{r By model}
d_comb %>%
  ggplot(aes(x=sg_score, y=delta_test_metric)) +
